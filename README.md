...
Adapter converts interface of some class to other interface that is
compatible with client requirements. It allows two classes work together
which could not cooperate before due to interface incompatibility.
...
Facade pattern keeps the complexities of the 
system from the client and provides an interface
which the client can access the system. Facade comes under 
structural pattern. 

This pattern involves a single class which provides simplified 
API required by client which makes usage of system much easier.
